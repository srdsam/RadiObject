...
class VolumeCollectionWriter:
    ...
    Examples:
        Uniform shape collection:
            ...
            with VolumeCollectionWriter(uri, shape=(256, 256, 128)) as writer:
        ...
        Heterogeneous shape collection (raw ingestion):
            ...
            with VolumeCollectionWriter(uri) as writer:
                for nifti_path, subject_id in niftis:
                    data = load_nifti(nifti_path)
                    writer.write_volume(data, obs_id=f"{subject_id}_T1w", obs_subject_id=subject_id)
    """
...
class RadiObjectWriter:
    ...
    adding collections one at a time via VolumeCollectionWriter instances.
    ...
    def add_collection(
        ...
        """Add a new collection and return a VolumeCollectionWriter for it.
        ...
        writer = _ManagedCollectionWriter(
            uri=collection_uri,
            shape=shape,
...
class _ManagedCollectionWriter(VolumeCollectionWriter):
    """VolumeCollectionWriter that registers with parent RadiObjectWriter on completion."""
