...
class Dataframe:
    ...
    Used internally for obs_meta (subject-level, 1-dim) and obs (volume-level, 2-dim) storage.
    Index dimensions are configurable via create()/from_pandas() and read dynamically from schema.
    ...
    def shape(self) -> tuple[int, int]:
        ...
        with tiledb.open(self.uri, "r", ctx=self._effective_ctx()) as arr:
            ...
            if isinstance(n_rows, str):
                n_rows = len(arr.query(attrs=[])[:][self.index_columns[0]])
    ...
    def all_columns(self) -> list[str]:
        ...
        return list(self.index_columns) + self.columns
    ...
    def __len__(self) -> int:
        with tiledb.open(self.uri, "r", ctx=self._effective_ctx()) as arr:
            result = arr.query(attrs=[])[:][self.index_columns[0]]
            return len(result)
    ...
    def read(
        ...
        idx_cols = self.index_columns
        if columns is not None:
            attrs = [c for c in columns if c not in idx_cols]
        else:
            attrs = self.columns
        with tiledb.open(self.uri, "r", ctx=self._effective_ctx()) as arr:
    ...
    def _validate_schema(
        ...
        index_columns: tuple[str, ...] = INDEX_COLUMNS,
    ) -> None:
    ...
    def _invalidate_cache(self) -> None:
        ...
        for prop in ("_schema", "dtypes", "index_columns"):
    ...
    def add_column(
        ...
        if fill is not None and len(self) > 0:
            with tiledb.open(self.uri, "r", ctx=ctx) as arr:
                all_data = arr[:]
                idx_cols = self.index_columns
                coords = tuple(all_data[dim] for dim in idx_cols)
                data = {col: all_data[col] for col in self.columns if col != name}
            data[name] = np.full(len(coords[0]), fill, dtype=np.dtype(dtype))
    ...
    def update(self, df: pd.DataFrame) -> None:
        ...
        attr_cols = [c for c in df.columns if c not in idx_cols]
        ...
        ctx = self._effective_ctx()
        coords = tuple(df[dim].astype(str).to_numpy() for dim in idx_cols)
        ...
        if missing_attrs:
            with tiledb.open(self.uri, "r", ctx=ctx) as arr:
                existing = arr.query(attrs=missing_attrs)[coords]
                for col in missing_attrs:
                    raw = existing[col]
                    if len(raw) == len(coords[0]):
                        data[col] = raw
                    else:
                        dt = self.dtypes[col]
                        data[col] = np.zeros(len(coords[0]), dtype=dt)
    ...
    def create(
        ...
        cls._validate_schema(schema, index_columns=index_columns)
        effective_ctx = ctx if ctx else get_tiledb_ctx()

        dims = [tiledb.Dim(name=col, dtype="ascii", ctx=effective_ctx) for col in index_columns]
    ...
    def from_pandas(
        ...
        dataframe = cls.create(uri, schema=schema, ctx=ctx, index_columns=index_columns)

        effective_ctx = ctx if ctx else get_tiledb_ctx()
        with tiledb.open(uri, mode="w", ctx=effective_ctx) as arr:
            coords = tuple(df[col].astype(str).to_numpy() for col in index_columns)
            data = {col: df[col].to_numpy() for col in attr_cols}
            arr[coords] = data
