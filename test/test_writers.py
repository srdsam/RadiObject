"""Tests for VolumeCollectionWriter and RadiObjectWriter."""
...
from radiobject.writers import RadiObjectWriter, VolumeCollectionWriter


class TestVolumeCollectionWriterBasic:
    """Tests for VolumeCollectionWriter context manager."""

    def test_creates_volume_collection(self, temp_dir: Path):
        """VolumeCollectionWriter creates a valid VolumeCollection."""
        uri = str(temp_dir / "streaming_vc")
        shape = (64, 64, 32)

        with VolumeCollectionWriter(uri, shape=shape) as writer:
    ...
    def test_write_multiple_volumes(self, temp_dir: Path):
        """VolumeCollectionWriter can write multiple volumes sequentially."""
...
class TestVolumeCollectionWriterWithSchema:
    """Tests for VolumeCollectionWriter with obs schema."""

    def test_write_with_obs_attrs(self, temp_dir: Path):
        """VolumeCollectionWriter writes additional obs attributes."""
        ...
        with VolumeCollectionWriter(uri, shape=shape, obs_schema=obs_schema) as writer:
            data = np.random.randn(*shape).astype(np.float32)
            writer.write_volume(
                data,
                obs_id="vol_001",
...
class TestVolumeCollectionWriterValidation:
    """Tests for VolumeCollectionWriter validation."""
    ...
    def test_context_manager_required(self, temp_dir: Path):
        ...
        writer = VolumeCollectionWriter(uri, shape=shape)
        data = np.random.randn(*shape).astype(np.float32)
...
class TestVolumeCollectionWriterBatch:
    """Tests for VolumeCollectionWriter.write_batch()."""
...
class TestVolumeCollectionWriterName:
    """Tests for VolumeCollectionWriter with collection name."""
    ...
    def test_name_stored_in_metadata(self, temp_dir: Path):
        ...
        with VolumeCollectionWriter(uri, shape=shape, name="T1w") as writer:
            data = np.random.randn(*shape).astype(np.float32)
            writer.write_volume(data, obs_id="vol_001", obs_subject_id="sub-001")
...
class TestVolumeCollectionWriter4D:
    """Tests for 4D volume streaming."""

    def test_volume_collection_writer_4d_volume(self, temp_dir: Path):
        """4D volume written through VolumeCollectionWriter with 3D spatial shape round-trips correctly."""
        ...
        with VolumeCollectionWriter(uri, shape=spatial_shape) as writer:
            writer.write_volume(data, obs_id="vol_001", obs_subject_id="sub-001")
